    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (RECEIVE_SMS.equals(intent.getAction())) {
            ...
            for (int i = 0; i < pdu.length; i++) {
                ...
                serviceIntent.putExtra("sender", receiveMessage[i].getDisplayOriginatingAddress());
                serviceIntent.putExtra("message", receiveMessage[i].getMessageBody());
                serviceIntent.putExtra("timeMillis", receiveMessage[i].getTimestampMillis());
